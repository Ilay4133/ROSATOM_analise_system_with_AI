...
project1 = 78629
t_interval1 = rp.TimeInterval(start = 0, end = 1000)
app1 = rp.Application(user)
model1 = app1.get_exploration_model(project1, t_interval1)
variables1 = pd.Series({'DN1000' : 1, 'DN600x17': 0.596, 'TANKPRESS':1e5, 'AREA_FITTING_23V1':0.13900896800234608,
                       'DN600x25': 0.58, 'HOLEAREADESIGN': 0.005281017250684443, 'DN300x13': 0.299,
                       'ROD_POSITION': 1, 'DIAMETER_FITTING_23V2': 0.27075712741610936,
                       'FILTER_RESIST_COEFF':139.15303122952462,'DIAMETER_FITTING_123':0.5459261228146222,
                       'SATURATED_PRESSURE': 90.945e3, 'DN800x9':0.802,'WATER_DENSITY':960,
                       'TANKTEMP':97}, dtype = float)

def getting_values_from_api1(model1) -> list:
    with model1 as md:
      md.run(variables1)
...
def start_api1():
    data_list=getting_values_from_api1(model1)
    return data_list
